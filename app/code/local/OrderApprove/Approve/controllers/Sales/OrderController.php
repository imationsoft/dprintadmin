...
class OrderApprove_Approve_Sales_OrderController extends Mage_Adminhtml_Sales_OrderController
{
    /**
        ...
        if ($order = $this->_initOrder()) {
            try {
               ...
               // $user->sendOrderUpdateEmail(true,"approve"); //added by wang 2015-7-7
                ...
                $this->_getSession()->addError($this->__('The order[ #'. $sum.'] was not approved successfully.'));
            }
            $this->_redirect('*/sales_order/view', array('order_id' => $order->getId()));
        }
    }
    ...
    public function rejectAction()         //added by Wang 2015-7-7
    {
        if ($order = $this->_initOrder()) {
            try {
                ...
                $this->_getSession()->addError($this->__('The order[ #'. $sum.'] was not rejected successfully.'));
            }
            $this->_redirect('*/sales_order/view', array('order_id' => $order->getId()));
        }
    }
    ...
    public function shipmentsAction()
    {
        ...
        $order->sendOrderUpdateEmail(true,"shipment");//addded by Wang 2015-7-6
//        $user = Mage::getSingleton('admin/session'); //added by wang 2015-7-7
//        $user->sendOrderUpdateEmail(true,"ship"); //added by wang 2015-7-7
    }
    ...
    public function massApproveAction()
    {
        ...
        foreach ($orderIds as $orderId) {
            ...
            if ($order->canCancel()) {
                ...
                $countNonApproveOrder++;
            }
        }
        if ($countNonApproveOrder) {
            if ($countNonApproveOrder) {
                $this->_getSession()->addError($this->__('%s order(s) cannot be approved', $countNonApproveOrder));
            } else {
                $this->_getSession()->addError($this->__('The order(s) cannot be approved'));
            }
        }
