 ...
 * @method string getApproveBeforeState()                               //added by Wang 2015-6-25
 ...
 */
class OrderApprove_Approve_Model_Order extends Mage_Sales_Model_Order
{
    /**
    ...
    /**
     * Retrieve order approve availability
     *
     * @return bool
     */
    public function canApprove()  //added by Wang 2015-6-25
    {
        $state = $this->getState();
        ...
        if ($this->getActionFlag(self::ACTION_FLAG_APPROVE) === false) {
            return false;
        }
        return true;
    }
    ...
    {
        if (!$this->canApprove()) {
            Mage::throwException(Mage::helper('sales')->__('Approve action is not available.'));
        }
    ...
    public function queueNewOrderEmail($forceMode = false)
    {
        ...
        //$mailer->setQueue($emailQueue)->send();changed by Wang 2015-6-30
        $mailer->send();
